...
| File | Purpose |
|------|---------|
| `~/dead_reckoning_odom.py` | Integrates /cmd_vel over time to estimate position, publishes odom->base_link TF at 50Hz |
...
SSH into the Jetson first in each terminal window:
...
**Terminal 1 - GPIO + motor controller**

```bash
sudo busybox devmem 0x2430068 w 0x5
sudo busybox devmem 0x2430070 w 0x5
sudo busybox devmem 0x243D048 w 0x5
sudo busybox devmem 0x2434098 w 0x5
sudo busybox devmem 0x24340A0 w 0x5
sudo busybox devmem 0x2434090 w 0x5
sudo busybox devmem 0x2430098 w 0x5
sudo busybox devmem 0x243D030 w 0x5
ros2 run mecanum_controller mecanum_node
```
...
```bash
ros2 launch slam_toolbox online_async_launch.py slam_params_file:=$HOME/slam_params.yaml use_sim_time:=false
```

**Terminal 6 - RViz** (open this SSH session with `ssh -X annaz@<jetson-ip>`)

```bash
rviz2
```

In RViz: Fixed Frame = `map`, add Map display on `/map`, add LaserScan display on `/scan`.

---

**Save the map** (run from any terminal once mapping is done)
